...
public class Formula {
	...
	public void FormulaStr(String calRule) {
		...
		if (checkValid()) {
			for (int i = 0; i < leftBracket; i++) {
				formulaStr = calRule.substring(calRule.lastIndexOf("(") + 1,
						calRule.indexOf(")")).trim();
				symbolParse(formulaStr);
				...
				calRule = calRule.substring(0, calRule.lastIndexOf("(")).trim()
						+ value
						+ calRule.substring(calRule.indexOf(")") + 1,
								calRule.length()).trim();
		...
		for (int i = 0; i < 4; i++) {
			...
			while (sym[i] != -1) {
				...
				if (containsLetter(insStr)) {
					insStr = Integer.toString(parseLetter(insStr,
							paras.get(count)));
				}
				list.add(insStr);
	...
	public static void main(String[] args) {
		String str = "(1+1+0.1*(n-1))*(n/2)* (500*0.01)";
		str = str.replace("n", "20");
		System.out.println(str);
		Formula test1 = new Formula(str);
